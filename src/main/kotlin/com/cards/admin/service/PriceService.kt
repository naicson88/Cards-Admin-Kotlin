...
import com.cards.admin.client.PriceClient
...
@Service
class PriceService(val priceClient: PriceClient) {

    var logger = LoggerFactory.getLogger(PriceService::class.java)

    fun updateDeckPrice(deckName: String) :List<PriceDTO> {
        logger.info(" Updating Deck Price: {}", deckName)
        val setNameFromApiList =getNameInPriceSetList(deckName)
        logger.info(" Name in API List: {} ", setNameFromApiList)

        val json = priceClient.getDeckPrice(deckName) ?: throw NoSuchElementException("Can't find Deck prices!")

        val obj = JSONObject(json)
        val jsonObj = obj.getJSONObject("data")
        val arrayCards = jsonObj.getJSONArray("cards") ?: throw NoSuchElementException("Cannot find Set price with name: $deckName")

        val listPrices: MutableList<PriceDTO> = mutableListOf()

        for((index, _) in arrayCards.withIndex()){
            val card: JSONObject = arrayCards.getJSONObject(index)
            val numbers = card.getJSONArray("numbers")
    ...
    fun createPriceObject(listPrices: MutableList<PriceDTO> , numbers: JSONArray, name: String):List<PriceDTO> {
        for((index, _) in numbers.withIndex()) {
            val numberData = numbers.getJSONObject(index)
            val priceData = numberData.getJSONObject("price_data")
            val status = priceData.getString("status")

            val price = if ("success" == status) {
                val data = priceData.getJSONObject("data")
                val prices = data.getJSONObject("prices")
                prices.getDouble("average")
            } else 0.0

            listPrices.add(createPriceDto(name, numberData, price))
    ...
    fun createPriceDto(name: String, numberData: JSONObject, price: Double) : PriceDTO {
        val dto = PriceDTO()
        dto.cardName = name
        dto.cardSetCode = numberData.getString("print_tag")
        dto.updateTime = Date()
        dto.cardRarity = numberData.getString("rarity")
        dto.price = price
        return dto
    }

    fun getNameInPriceSetList(originalName: String) : String {
        return PriceAPISetListNames.listSetNames.first { it.contains(originalName) }
    }

    fun updateCardPrice(cardName: String) : List<PriceDTO> {
        logger.info(" Updating Card Price: {}", cardName)

        val json = priceClient.getCardPrice(cardName) ?: throw NoSuchElementException("Can't find Card's prices!")
        val listPrices : MutableList<PriceDTO> = mutableListOf()
        val obj = JSONObject(json)
        val cardsArray = obj.getJSONArray("data")
        createPriceObject(listPrices, cardsArray, cardName)

        return listPrices
    }
}

class PriceAPISetListNames {
    companion object {
        val listSetNames = listOf("2-Player Starter Deck: Yuya & Declan",
